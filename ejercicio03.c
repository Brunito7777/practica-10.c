...
int main () 
{
    ...
    int i,j;
    int renglon,columna;
    printf("\nDa el número de renglones y columnas separados con coma, "); printf("cuyo valor no sea mayor a 10 "); scanf("%d,%d",&renglon,&columna);
    if(((renglon>=1) && (renglon<=10))&&((columna>=1) && (columna<=10))) 
    {
        ...
        for (i= 0 ; i <= renglon-1 ; i++) 
        {
            ...
            {
            printf("\nNúmero para el elemento [%d][%d] del arreglo ", i,j ); scanf("%d",&lista[i][j]);
            }
        }
